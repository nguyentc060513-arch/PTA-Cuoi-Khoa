...
class Login(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.cb_showPassword.stateChanged.connect(self.togglePassword)
        self.pb_back.clicked.connect(self.Leave)
    def Leave(self):
        self.resetForm()
        WelcomePage.show() # Hiển thị page 
        self.close() # Câu lệnh đóng page
    def resetForm(self):
        self.le_Lg_email.clear()
        self.le_Lg_password.clear()
    ...
    def togglePassword(self):
        if self.cb_showPassword.isChecked():
            self.le_Lg_password.setEchoMode(QLineEdit.EchoMode.Normal)
        else:
            self.le_Lg_password.setEchoMode(QLineEdit.EchoMode.Password)
...
class Register(QtWidgets.QMainWindow):
    ...
    def togglePassword(self):
        if self.cb_showPassword.isChecked():
            ...
            self.le_retypePassword.setEchoMode(QLineEdit.EchoMode.Normal)
        else:
            self.le_password.setEchoMode(QLineEdit.EchoMode.Password)
            self.le_retypePassword.setEchoMode(QLineEdit.EchoMode.Password)
...
class Welcome(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        uic.loadUi("./Lesson3/UI/Welcome.ui", self)

        self.pb_start.clicked.connect(self.OpenLogin)
        self.pb_leave.clicked.connect(self.Leave)

        # Tạo player phát âm thanh
        self.player = QMediaPlayer(self)
        self.audio_output = QAudioOutput(self)
        self.player.setAudioOutput(self.audio_output)

        # Khi nhạc phát xong thì thoát app
    ...
    def Leave(self):
        # Đường dẫn tới file Goodbye.mp3
        sound_path = os.path.abspath("./Lesson3/Sound/Goodbye.mp3")

        # Gán file nhạc
        self.player.setSource(QUrl.fromLocalFile(sound_path))
        self.audio_output.setVolume(0.8)  # Âm lượng 100%
        self.player.play()
